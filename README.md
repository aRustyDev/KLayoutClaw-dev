...
`install.py` creates `~/.klayout/klayoutclaw.json` on first install and
preserves it on later installs. For example, this keeps AnkiConnect on its
conventional port `8765` and runs KlayoutClaw on `8766`:
