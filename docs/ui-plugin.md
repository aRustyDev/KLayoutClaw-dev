...
The deprecated `python install.py` wrapper performs the same installation.
Both paths copy `klayoutclaw_server.lym` and `klayoutclaw_ui.lym` to
`~/.klayout/pymacros/` and create `~/.klayout/klayoutclaw.json` when it does not
already exist. Restart KLayout to activate.
